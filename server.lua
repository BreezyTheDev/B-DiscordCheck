...
RegisterCommand("sc", function(source, args, rawCommand)
    if IsPlayerAceAllowed(source, "StaffChat.View") then
        ...
        TriggerClientEvent("S-StaffChat:SendToStaffClient", -1, staffmessage, src, ifMessage)
    else
...
RegisterNetEvent("S-StaffChat:SendToStaff")
AddEventHandler("S-StaffChat:SendToStaff", function(message, id, checkMessage)
	local _src = source
...
RegisterServerEvent("S-StaffChat:checkPerms")
AddEventHandler("S-StaffChat:checkPerms", function()
    if IsPlayerAceAllowed(source, "StaffChat.View") then
        TriggerClientEvent("S-StaffChat:returnPerms", source, true)
    else
        TriggerClientEvent("S-StaffChat:returnPerms", source, false)
    end
end)
...
PerformHttpRequest("https://raw.githubusercontent.com/SmurkTheDev/S-StaffChat/main/fxmanifest.lua", function(error, res, head)
    ...
    if verFile then
        if tonumber(verFile) < resp then
            print("^1[^4" .. expectedName .. "^1] WARNING^0")
            print("^4" .. expectedName .. " ^0is outdated. Please update it from ^5https://github.com/SmurkTheDev/S-StaffChat^0| Current Version: ^1" .. verFile .. " ^0| New Version: ^2" .. resp .. " ^0|")
        ...
        print("^1[^4" .. expectedName .. "^1] WARNING^0")
        print("You may not have the latest version of ^4" .. expectedName .. "^0. A newer, improved version may be present at ^5https://github.com/SmurkTheDev/S-StaffChat^0")
    end
end)
